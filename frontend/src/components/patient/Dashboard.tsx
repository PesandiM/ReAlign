...
import {
  ...
  Switch, FormControlLabel, Tab, Tabs, Tooltip, Badge
} from '@mui/material';
...
const NAV_ITEMS = [
  { label: 'Overview',     path: '/patient',              icon: <DashboardIcon /> },
  { label: 'Appointments', path: '/patient/appointments', icon: <CalendarMonthIcon /> },
  { label: 'History',      path: '/patient/history',      icon: <HistoryIcon /> },
  { label: 'Profile',      path: '/patient/profile',      icon: <PersonIcon /> },
];

const Sidebar: React.FC<{ open: boolean; onClose: () => void; mobile: boolean }> = ({ open, onClose, mobile }) => {
  const navigate = useNavigate();
  const location = useLocation();
  const user = authService.getCurrentUser();

  const content = (
    <Box sx={{ width: SIDEBAR_W, height: '100%', display: 'flex', flexDirection: 'column', bgcolor: '#2c2e25' }}>
      {/* Brand */}
      ...
      <Box sx={{ px: 3, py: 2, borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <Avatar sx={{ bgcolor: '#6a8d73', width: 36, height: 36, fontSize: 14, fontWeight: 700 }}>
          ...
          <Box>
            ...
            <Chip label="Patient" size="small"
              sx={{ height: 16, fontSize: '10px', bgcolor: '#6a8d73', color: '#fff', mt: 0.3 }} />
          </Box>
        </Box>
      </Box>
          ...
          return (
            <ListItem key={item.path} disablePadding sx={{ px: 1.5, mb: 0.5 }}>
              <ListItemButton
                ...
                <ListItemIcon sx={{ minWidth: 36, color: active ? '#fff' : 'rgba(255,255,255,0.5)' }}>
                  {item.icon}
                </ListItemIcon>
      ...
      <Box sx={{ p: 2, borderTop: '1px solid rgba(255,255,255,0.08)' }}>
        <ListItemButton onClick={() => { authService.logout(); navigate('/'); }}
          sx={{ borderRadius: 2, '&:hover': { bgcolor: 'rgba(255,255,255,0.07)' } }}>
          <ListItemIcon sx={{ minWidth: 36, color: 'rgba(255,255,255,0.4)' }}>
            <LogoutIcon fontSize="small" />
          </ListItemIcon>
          <ListItemText primary="Sign Out"
            primaryTypographyProps={{ fontSize: 13, color: 'rgba(255,255,255,0.4)' }} />
        </ListItemButton>
      </Box>
    </Box>
  );
  ...
  if (mobile) {
    return (
      <Drawer open={open} onClose={onClose} PaperProps={{ sx: { bgcolor: '#2c2e25' } }}>
        {content}
      </Drawer>
    );
  }
  return (
    <Box sx={{ width: SIDEBAR_W, flexShrink: 0 }}>
      <Box sx={{ width: SIDEBAR_W, position: 'fixed', top: 0, left: 0, height: '100vh', overflow: 'auto' }}>
        {content}
      </Box>
    </Box>
  );
...
const ProfilePage: React.FC<{ patientId: string }> = ({ patientId }) => {
  ...
  useEffect(() => {
    patientService.getProfile(patientId)
      .then(p => { setProfile(p); setForm({ name: p.name, contact_no: p.contact_no, age: p.age, gender: p.gender }); })
...
const PatientDashboard: React.FC = () => {
  ...
  // Get patient_id — stored in user object or derive from userId
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: '#f7f7f5' }}>
      ...
      <Box sx={{ display: { xs: 'none', md: 'block' } }}>
        <Sidebar open mobile={false} onClose={() => {}} />
      </Box>

      {/* Mobile sidebar */}
      <Box sx={{ display: { xs: 'block', md: 'none' } }}>
        <Sidebar open={mobileOpen} mobile onClose={() => setMobileOpen(false)} />
      </Box>
      ...
      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <Box sx={{
          ...
          px: 2, py: 1.5, bgcolor: '#2c2e25', position: 'sticky', top: 0, zIndex: 100,
        }}>
